...
print("""
 ...
 |_____/|_| \_|_____/  |_|  \_\___|\___\___/|_|  \__,_|___/ |_____/ \__,_|_| |_| |_| .__/ \___|_|   
					...
					Author:n0b1ta 
					Github:https://github.com/N0b1ta/DNSRecordsDumper
...
def main():
    ...
    for targetentry in c.response:
        ...
        for record in targetentry['raw_attributes']['dnsRecord']:
            ...
            continue
    with codecs.open('DnsRecords.csv', 'w', 'utf-8') as outfile:
        outfile.write('type,name,value\n')
        for row in outdata:
            outfile.write('{type},{name},{value}\n'.format(**row))
            print(row)
	print_o('Found %d records,check File: DnsRecords.csv' % len(outdata))
